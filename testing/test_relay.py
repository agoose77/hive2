...
def build_myhive(i, ex, args):
   i.mod = hive.modifier(lambda self: print("WOOO"))
   i.relay = hive.relay()
   ex.do = hive.entry(i.relay)

   hive.trigger(i.relay, i.mod)
   hive.trigger(i.relay, i.mod2)
   hive.trigger(i.relay, i.mod3, pretrigger=True)
